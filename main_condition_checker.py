...
import csv
import os
os.makedirs("mid_spin_sols_2", exist_ok=True)
...
with open("mid_spin_data_2.csv", "w", newline="") as f:
    writer = csv.writer(f)
                        ...
                        for lifetime in [1000, 10000, 1e5, 1e6]:
                            ...
                            for a0 in [2*a_min_si, 3*a_min_si, 4*a_min_si, 5*a_min_si]:
                                if a0 <= a_max:
                                    ...
                                    if T_TF <= 1000*lifetime:  # assume if it's bigger then it has no hope
                                        ...
                                        for Omega0 in [1e-5]:
                                            ...
                                            if f[-1] <= 2*fmin:
                                                ...
                                                if a_spin <= 0.3:
                                                    ...
                                                    np.savez(
                                                        f"mid_spin_sols_2/{sol_id}.npz", t=t, f=f, Omega=Omega)
                                                    counter += 1
