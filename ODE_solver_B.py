...
def solve_Radau_4(dxdt, dydt, dzdt, dwdt, x0, y0, z0, w0, t0, tfinal, x_scale, y_scale, z_scale, w_scale, t_scale):
    from scipy.integrate import solve_ivp
    import numpy as np

    def system_scaled(t_scaled, vars_scaled):
        # unpack all 4 scaled variables
        x_scaled, y_scaled, z_scaled, w_scaled = vars_scaled
        ...
        # calculate physical derivatives and scale them for the solver
        dx_scaled_dt_scaled = dxdt(x, y, z, w) * t_scale / x_scale
        dy_scaled_dt_scaled = dydt(x, y, z, w) * t_scale / y_scale
        dz_scaled_dt_scaled = dzdt(x, y, z, w) * t_scale / z_scale
        dw_scaled_dt_scaled = dwdt(x, y, z, w) * t_scale / w_scale
        
        return [dx_scaled_dt_scaled, dy_scaled_dt_scaled, dz_scaled_dt_scaled, dw_scaled_dt_scaled]
        
    # initial conditions (scaled)
    u0_scaled = [x0 / x_scale, y0 / y_scale, z0 / z_scale, w0 / w_scale]
    
    # time span and evaluation points (scaled)
    t_span = (t0 / t_scale, tfinal / t_scale)
    t_eval = np.linspace(t_span[0], t_span[1], 500)
    
    # run the Radau solver
    sol_scaled = solve_ivp(system_scaled, t_span, u0_scaled,
                           t_eval=t_eval, method='Radau')
                           
    # unscale the results back to physical units
    sol_t = sol_scaled.t * t_scale
    sol_x = sol_scaled.y[0] * x_scale
    sol_y = sol_scaled.y[1] * y_scale
    sol_z = sol_scaled.y[2] * z_scale
    sol_w = sol_scaled.y[3] * w_scale
    
    return sol_t, sol_x, sol_y, sol_z, sol_w
